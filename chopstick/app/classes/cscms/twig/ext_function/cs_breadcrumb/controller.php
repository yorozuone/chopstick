<?php
namespace app\cscms\twig\ext_function\cs_breadcrumb;
...
class controller extends \app\cscms\twig\ext_function
{
    ...
    public static function display($config = array())
    {
        ...
        $vars = array
        (
            'rs_breadcrumb' => self::rs_breadcrumb(\app\cscms\model\helper\page::get_current_page_id()),
        );
        $vars = array_merge_recursive($config, $vars);
        $view = new view();
        return $view->render('/cscms/twig/ext_function/cs_breadcrumb/'.$config['template'].'.twig', $vars);
    }
    // --------------------------------------------------------------------------------
    //
    // --------------------------------------------------------------------------------
