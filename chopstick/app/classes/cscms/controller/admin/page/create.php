...
// dataset
use \app\cscms\model\controller\admin\page\create as dataset_page;
...
class create extends \app\controller_admin
{
    ...
    public function action_entry($params)
    {
        ...
        {
            response::redirect(url::create('/cscms/admin/page/create', array($this->dataset_page->get_value('parent_page_id'))));
        }
        $this->dataset_page->create();       
        response::redirect(url::create('/cscms/admin/page/edit', array($this->dataset_page->get_value('page_id'))));
    }
    // ********************************************************************************
    // **** 表示
    // ********************************************************************************
    ...
    public function display()
    {
        $vars = array
        (
            'rs_composer' => \app\cscms\model\recordset\composer::fetch_all(),
            'values' => $this->dataset_page->get_values(),
        );
        echo $this->render('cscms/controller/admin/page/create/composer.twig', $vars);
    }
}